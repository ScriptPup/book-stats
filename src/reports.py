...
class Reports:
    ...
    def badwords_by_chapter(self
        ...
        """        
        if not axes: axes = plt.gca() 
        data  = [chapt.stats.badword_count for chapt in self.chapters]        
        labels = self.get_chapter_axis_labels(titled_by)
        chart = self.plot_chart_type(axes, data, labels, chart_type=chart_type, **chart_args)
        axes.set_xticks(axes.get_xticks())
        axes.set_xticklabels(labels,**xaxes_options)
    ...
    def get_chapter_axis_labels(self, titled_by: str="name"):
        titled_by = titled_by.lower()
        if titled_by == "name":
            return [chapt.name for chapt in self.chapters]
        else:
            return [f"Chapter {chapt.number}" for chapt in self.chapters]
